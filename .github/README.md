...
Tracked config lives under **`~/.dotfiles/`** so `yadm clone` does not drop `Brewfile` or other dotfiles in `$HOME`. Repo docs live in **`~/.github/README.md`** (GitHub’s supported path for the landing-page README). Bootstrap symlinks shell/editor config into the usual paths.
...
yadm checks out into `$HOME` (`.gitignore`, `.github/README.md`, `.dotfiles/`, and `.config/yadm/bootstrap`), then runs bootstrap:
...
| Path | Purpose |
|------|---------|
| `~/.github/README.md` | Repo docs (GitHub landing-page README) |
